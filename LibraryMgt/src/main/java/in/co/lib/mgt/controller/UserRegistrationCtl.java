...
public class UserRegistrationCtl extends BaseCtl {
		...
		if (OP_SIGN_UP.equalsIgnoreCase(op)) {
			...
			//	System.out.println("in try sign up");
				...
				if(pk>0) {
					...
					ServletUtility.forward(LIBView.STUDENT_DETAIL_VIEW, request, response);
					
				}else {
					request.getSession().setAttribute("UserBean", bean);
					ServletUtility.setErrorMessage("User Not  Registered Try Again", request);
					ServletUtility.forward(LIBView.USER_REGISTRATION_VIEW, request, response);	
				}
			...
			ServletUtility.redirect(LIBView.USER_REGISTRATION_CTL, request, response);
			return;
		}
	...
	protected String getView() {
		return LIBView.USER_REGISTRATION_VIEW;
	}
